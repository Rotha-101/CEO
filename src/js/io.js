...
export async function makePreviewReport(ev) {
  try {
    ...
    styleEl.textContent = managementCss;
    clonedHtml.querySelector('head').appendChild(styleEl);

    // Remove backdrop if it was open
    const bd = clonedHtml.querySelector('#backdrop');
    if (bd) bd.remove();

    // Ensure preview is visible (also handled by CSS, but good measure)
